<?php

  //List of roster
	$i = 0;
	$roster = array();
    // admins ----------START
	$roster[$i]['nome'] = "n3m3s1s";
	$roster[$i]['img'] = "img/roster/n3m3s1s.jpg";
	$roster[$i]['desc'] = "E' il clan leader, e proprio per questo, ha sempre ragione lui. Se lui tira una nade al gate, e tu ci vai addosso, la colpa &egrave tua";
	$roster[$i]['cit'] = "DEMONIO LADRO IL LASER!!";
	$i++;

	$roster[$i]['nome'] = "Zamy";
	$roster[$i]['img'] = "img/roster/zamy.jpg";
	$roster[$i]['desc'] = "Conosciuto grazie alla sua abilit&agrave innata: GOD MODE. Attraversa il narrow, il patio, il market, il new, ritornando alla fontana ancora con 100 hp mietendo chiunque trovi per strada";
	$roster[$i]['cit'] = "";
	$i++;

	$roster[$i]['nome'] = "Heresyarch/Tsujigiri";
	$roster[$i]['img'] = "img/roster/nic.jpg";
	$roster[$i]['desc'] = "";
	$roster[$i]['cit'] = "";
	$i++;
    // admins ------------STOP
    // soldiers ----------START
	$roster[$i]['nome'] = "Revan/Yuu(omosessuale)";
	$roster[$i]['img'] = "img/roster/revan.jpg";
	$roster[$i]['desc'] = "L'uomo che ci proverebbe anche con la Montalcini, quest'essere met&agrave veneto e met&agrave rosik &egrave la colonna portante di tutte le vittorie dei 2s2h... o almeno cos&igrave sostiene.";
	$roster[$i]['cit'] = "Abbiamo vinto solo perch&egrave c'ero io";
	$i++;

// 	$roster[$i]['nome'] = "Kolbebo";
// 	$roster[$i]['img'] = "img/roster/kolbebo.jpg";
// 	$roster[$i]['desc'] = " La prima donna ad essere entrato nel clan.<br/><br/>Regina dello spam e delle cazzate.<br/><br/>Si diletta facendo scherzi un p&ograve; a tutti in particolare al boss, n3m3s1s.";
// 	$roster[$i]['cit'] = "\"Do what you want cause a pirate is free, you are a pirate! Le Gocciole sono I biscotti migliori del mondo, I <3 Gocciole\" ";
// 	$i++;

// 	$roster[$i]['nome'] = "Roger";
// 	$roster[$i]['img'] = "img/roster/roger.jpg";
// 	$roster[$i]['desc'] = "";
// 	$roster[$i]['cit'] = "";
// 	$i++;

// 	$roster[$i]['nome'] = "Arca";
// 	$roster[$i]['img'] = "img/roster/arca.jpg";
// 	$roster[$i]['desc'] = " Donna che incute timore nelle sue prede.<br/><br/>Mafiosa e alcolizzata, questa ragazza tiene testa a molti sbruffoni che si trovano in giro.";
// 	$roster[$i]['cit'] = "\"Sono rossa, ubriacona e cattiva. Nel mio tempo libero prendo per il culo Revan e cerco di fargli ammettere l'omosessualit&agrave; e il ritardo mentale\" ";
// 	$i++;

	$roster[$i]['nome'] = "Engine";
	$roster[$i]['img'] = "http://dailynade.com/wp-content/uploads/2009/07/me-150x150.jpg";
	$roster[$i]['desc'] = "Anche chiamato 'quickzoom' &egrave colui che predilige l'utilizzo del sr8 all'interno del clan.";
	$roster[$i]['cit'] = "~ Ratto, impara l'italiano che &egrave meglio<br/>~ <durante CW rispondendo al cell>Ah ciao Giulia. Ti chiamo tra 8 minuti<br/>~ <durante CW rispondendo al cell>Ciao giulia, oggi non posso, ho CW";
	$i++;

	$roster[$i]['nome'] = "Ranker";
	$roster[$i]['img'] = "img/roster/ranker.jpg";
	$roster[$i]['desc'] = "Questo nerdone si &egrave; autoproclamato il simpaticone del clan (almeno, cos&igrave; mi dice la Kolbebo).<br/><br/>Dotato di una fortissima \"R\" moscia, egli fa copia con Rambo, due elementi presi spesso di mira da Arca";
	$roster[$i]['cit'] = "\"Ho un fratello gemello e sono molto ma molto ma molto bello!\"";
	$i++;

// 	$roster[$i]['nome'] = "S3if3r";
// 	$roster[$i]['img'] = "img/roster/seifer.jpg";
// 	$roster[$i]['desc'] = " Chitarrista e metallaro italo-inglese.<br/><br/>Il segugio del clan sempre pronto a fiutare noobs e cheaters sul server.<br/><br/>
// La sua arte: Seek and Destroy!";
// 	$roster[$i]['cit'] = "\"M K KZZ V?? T SPKK L KL STRNZ! T MDR ZKKL!\"";
// 	$i++;

    $roster[$i]['nome'] = "Ratchet";
    $roster[$i]['img'] = "img/roster/ratchet.jpg";
    $roster[$i]['desc'] = "Direttamente dai tunnel pi&ugrave profondi dell'etna questo piccolo topo sa farsi valere in battaglia anche se le apparenze dicono il contrario.";
    $roster[$i]['cit'] = "Neme, ma che cazzo lanci la nade l&igrave a fare??<br/>..<br/>";
    $i++;

//     $roster[$i]['nome'] = "Rambo";
//     $roster[$i]['img'] = "img/roster/rambo.jpg";
//     $roster[$i]['desc'] = "l'uomo che dimentic&ograve; la R nell'utero di sua madre insieme all'abbronzatura ( cit. Arca )";
//     $roster[$i]['cit'] = "";
//     $i++;
/*
    $roster[$i]['nome'] = "SerasVictoria";
    $roster[$i]['img'] = "img/roster/anonymous.jpg";
    $roster[$i]['desc'] = "";
    $roster[$i]['cit'] = "";
    $i++;*/

    // soldiers ------------STOP

//     $roster[$i]['nome'] = "Savo";
//     $roster[$i]['img'] = "img/roster/savo.jpg";
//     $roster[$i]['desc'] = "Ex bimbominchia che grazie alla protezione di Heresyarch &egrave; riuscito a crescere e a diventare una persona seria (o quasi)";
//     $roster[$i]['cit'] = "\"daiiiii neme, accetta la mia amicizia su facebook\" ";
//     $i++;
// 
//     $roster[$i]['nome'] = "Burzum";
//     $roster[$i]['img'] = "img/roster/anonymous.jpg";
//     $roster[$i]['desc'] = "";
//     $roster[$i]['cit'] = "";
//     $i++;


?>